...
## Workstation setup file based on one found in the history of the autorest git repository. https://github.com/azure/autorest 
#
# It is extremely likely this file will need to be adjusted as versions and tools change over time.
...
if( !(get-command -ea 0 node) ) { 
    ...
    invoke-webrequest "https://github.com/coreybutler/nvm-windows/releases/download/1.1.7/nvm-noinstall.zip" -outfile  "c:\tmp\nvm.zip" 
    ...
    if( !(get-command -ea 0 node) ) { return write-error "No NodeJS in PATH." }
...
if( !(get-command -ea 0 pip) ) { 
    ...
    ( New-Object System.Net.WebClient).DownloadFile("https://www.python.org/ftp/python/3.7.5/python-3.7.5-amd64.exe","c:\tmp\python-3.7.5-amd64.exe" )

    if( !(test-path -ea 0  "c:\tmp\python-2.7.12.amd64.msi") ) { return write-error "Unable to download Python 2.7" }
    if( !(test-path -ea 0  "c:\tmp\python-3.7.5-amd64.exe") ) { return write-error "Unable to download Python 3.7" }
    ...
    write-host -fore darkcyan "      Installing Python 3.7."
    C:\tmp\python-3.7.5-amd64.exe /quiet InstallAllUsers=1 PrependPath=1
    while( (get-process -ea 0 python*) )  { write-host -NoNewline "." ; sleep 1 }
    ReloadPathFromRegistry
    if( !(get-command -ea 0 pip) ) { return write-error "No PYTHON in PATH." }
}
...
if( !(get-command -ea 0 go.exe) ) { 
    ...
    invoke-webrequest "https://dl.google.com/go/go1.13.4.windows-amd64.msi" -outfile  "c:\tmp\go1.13.4.windows-amd64.msi" 
    if( !(test-path -ea 0  "c:\tmp\go1.13.4.windows-amd64.msi" ) ) { return write-error "Unable to download Go" }
    write-host -fore darkcyan "      Installing Go."
    Start-Process -wait -FilePath msiexec -ArgumentList  "/i", "C:\tmp\go1.13.4.windows-amd64.msi", "/passive"
    ReloadPathFromRegistry
...
#$p = ([System.Environment]::GetEnvironmentVariable( "path", 'Machine'))
#$p = "$p;C:\Users\username\tools;"
#$p = "$p;C:\Users\username\.crc\bin;"
#([System.Environment]::SetEnvironmentVariable( "path", $p,  'Machine'))
